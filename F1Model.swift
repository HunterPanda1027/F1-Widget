...
class F1Service {
    ...
    func fetchNextMeeting() async throws -> F1Meeting? {
            ...
            do {
                ...
                // 1. Check if OpenF1 is blocking us
                ...
                if upcomingMeeting == nil {
                    print("⚠️ Successfully fetched data, but no future races were found based on the date!")
                } else {
                    print("✅ Found upcoming race: \(upcomingMeeting?.meetingName ?? "Unknown")")
                }
                
                return upcomingMeeting
                
            } catch {
                // 3. Catch and print the EXACT error
                print("❌ Error in fetchNextMeeting: \(error)")
                return nil
            }
        }
